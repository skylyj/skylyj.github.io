...
** 主要内容
- 主要内容
  + 从高阶语义向量的组合表达来看attention $\mathcal{A}(X_i) &=  \dfrac{\sum_{j=1}^{N} sim(Q_i,K_j) V_j}{\sum_{j=1}^N sim(Q_i,K_j)}$
  + 提供transformer 的代码讲解
  + 从kernel的角度来看attention $O(N^2)$ 的计算量源于softmax的存在
  + linear attention
- 参考
