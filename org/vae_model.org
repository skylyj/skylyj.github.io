...
- $\log P(X) \ge \mathcal{L} (Q)$ ，由于 KL 非负
  ...
  + 参数化: $Q_{\phi}(Z)$  $\max\limits_{\phi}\mathcal{L}(Q)$ (变分的思想)
  + $\mathcal{L} (Q)$ 就是 ELBO
*** 证明
$\begin{aligned}
...
** 参数化和求解ELBO
给定一个$X$
- $\mathcal{L} = \mathrm{E}_{Q(Z)}[\log P(X|Z)] - \mathcal{D}[Q(Z) \| P(Z)]$
...
- 第一项利用MC 近似期望
   + $\mathrm{E}_{Q_{\phi}(Z|X))}[\log P_{\theta}(X|Z)]
- 第二项基于显式的计算来求导
   $\begin{aligned}
