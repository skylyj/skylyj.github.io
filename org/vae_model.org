...
- Product: if p(x) and p(z) are Gaussian, then p(x)p(z) is proportional to a Gaussian
  ...
  - diffusion model 中会使用到

     $P(z_{t-1})$ 是高斯，$P(z_t|z_{t-1})$ 也是高斯，那么$P(z_{t-1}|z_{t})$ 也是高斯。
- x gaussian N(0,1), y gaussian N(0,1), ax+by gaussion, with sigma= a^2+b^2.
*** 性质
...
*** 概率中的表述：
如果 $X$ 是随机变量，$\phi$ 是一个凸函数的话，r$\phi (E(X)) \le E(\phi(X))$
*** 注
- log函数是一个凹函数,  $\log (E(X)) \ge E(\log(X))$
- 如果遇到 log和期望的时候，我们可以让log穿过期望符号，得到一个好的下界
   + $\log E_{Q(Z)} \dfrac{P(X,Z)}{Q(Z)} dZ \ge E_{Q(Z)}\log\dfrac{P(X,Z)}{Q(Z)}$
** 变分
*** 泛函 fuctional
- 输入是一个函数，输出一个值
...
*** 每个图像都是一个高维的随机变量
- 是可以sample的
- 可以认为每个图像都是在一个独特的分布下采样的结果
  + 按照像素点的当前值，是一个高斯分布，有唯一的均值，不过方差为0
  + 如果我们把方差relax以下，我们也可以基于每个图像得到一些新的图像
- 从一图一个分布到一个编码一个分布
  + 一个图像一个分布有点太粗糙了
  + 对图像$X$ 来做编码$Z$ ，如果编码相同，那么对应的分布是P(X|Z)是相同的。
  + 这套编码至关重要，有了它，图像的基本特征就决定了，
    + 人脸中，它可能包括了性别，肤色，发型，脸型等等
  + 如果把 $P(X|Z)$ 学好了，那么我直接采样 $Z$ 就好了。
    + 我希望直接从 $Z$ 中来采样
    + 这个分布很容易sample，首选的还是多维的gaussian 分布
